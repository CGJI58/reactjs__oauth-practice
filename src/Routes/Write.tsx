  ...
  useEffect(() => {
    if (!diaryState.ready) {
      ...
      if (isWriteOption(rawMode)) {
        ...
        if (rawMode === "create") {
          const restoredTitle = sessionStorage.getItem("tempTitle");
          const restoredText = sessionStorage.getItem("tempText");
          if (restoredTitle && restoredText) {
            setValue("title", restoredTitle);
            setValue("text", restoredText);
          }
        }
  ...
  const tempSave = debounce(
    ({ title, text }: { title: string; text: string }) => {
      const okToSave: boolean = !(
        ...
        isEqual(
          { title, text },
          { title: originalDiary.title, text: originalDiary.text }
        )
      );
      if (okToSave) {
